...
- In the “高亮口播” template, treat the white category label, yellow subtitle, green-outlined headline, pink highlight, and account signature as independent layers. Operators must be able to select or directly drag each layer and adjust its horizontal/vertical position, width, scale, font size, font family, and alignment independently, with per-layer and full-template reset actions.
